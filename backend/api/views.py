...
def wishlist_manager(request):
    ...
    if request.method == 'POST':
        ...
        item, created = Wishlist.objects.get_or_create(session_id=session_id, product=product)
        if not created:
            item.delete()
    ...
    if request.method == 'DELETE':
        product_id = request.data.get('product_id')
        Wishlist.objects.filter(session_id=session_id, product_id=product_id).delete()
